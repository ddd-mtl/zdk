...
pub fn get_latest_time_indexed_links(
   ...
   if items_limit == 0 {
      return Err(wasm_error!(WasmErrorInner::Guest(
         "Invalid input for get_latest_time_indexed_links(): 'items_limit' cannot be '0'".to_string()
      )));
   }
   ...
   if latest_hour_tp.exists()? {
      ...
      let last_hour_links = get_links(lquery, strategy)?;
      debug!("latest_hour_tp items: {}", last_hour_links.len());
      let mut last_hour_pairs = last_hour_links
         .into_iter()
      ...
      if total_items.len() >= items_limit {
         ...
         if prev_hour_tp.exists()? {
            ...
            lquery.tag_prefix = link_tag.clone();
            let prev_hour_links = get_links(lquery, strategy)?;
            debug!("prev_hour_tp items: {}", prev_hour_links.len());
            let mut prev_hour_pairs = prev_hour_links
               .into_iter()
   ...
   while total_items.len() < items_limit && current_sweep_tp.as_ref().len() >= root_anchor_tp.as_ref().len() {
      ...
      if current_sweep_tp.exists()? {
         ...
         let current_search_time_us =
            convert_timepath_to_timestamp(current_sweep_tp.path.clone()).unwrap_or(rounded_sweep_interval.begin); // If at root component, search years starting from begin_time
         ...
         if !older_children_pairs.is_empty() {
            ...
            debug!(
               "*** Starting recursive sweep of the {} descendants of {} (depth {})",
               older_children_pairs.len(),
               timepath2anchor(&current_sweep_tp),
               depth
            );
            sweep_and_append(
            ...
            debug!(
               "Descendants of {} (depth {}). Leafs added {}",
               timepath2anchor(&current_sweep_tp),
               depth,
               /*raw_children_dbg_info,*/ links_added.len()
            );
         }
      }
   ...
   debug!(
      "current_sweep_tp = {} | total = {}",
      timepath2anchor(&current_sweep_tp),
      total_items.len()
   );
   let oldest_probed_bucket_time = if total_items.len() >= items_limit {
      if has_probed_prev {
         prev_hour_us
      } else {
      ...
      convert_timepath_to_timestamp(current_sweep_tp.path).unwrap_or(rounded_sweep_interval.begin)
   };

   debug!(
      "END. sweeped_interval = [{}, {}]",
      ts2anchor(oldest_probed_bucket_time),
      ts2anchor(rounded_sweep_interval.end)
   );
...
fn sweep_and_append(
   ...
   for (parent_tp, compi32, child_link) in children {
      if depth == 0 {
         ...
         lquery.tag_prefix = link_tag.clone();
         let links = get_links(lquery, strategy)?;
         ...
         let mut pairs = links.into_iter().map(|l| (ts, l)).collect();
