...
pub fn get_eh(ah: ActionHash, strategy: GetStrategy) -> ExternResult<EntryHash> {
   ...
   let Some(eh) = record.action().entry_hash() else {
      return zome_error!(
         "{}",
         format!("Record does not hold an EntryHash. {}", record.action_address())
      );
   };
   Ok(eh.to_owned())
}
...
pub fn get_typed_from_record<T: TryFrom<Entry>>(record: Record) -> ExternResult<T> {
   ...
   let err = error::<T>(&format!("Converting Entry to type failed for entry: {:?}", entry))
      .err()
      .unwrap();
...
///
pub fn get_app_entry_name_from_def(
...
pub fn get_latest_entry(target: EntryHash, options: GetOptions) -> ExternResult<Option<Entry>> {
   ...
   let sah = updates
      .into_iter()
      .fold(None, |latest: Option<SignedActionHashed>, update| match latest {
         Some(latest) => {
