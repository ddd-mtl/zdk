...
pub fn get_local_from_eh(eh: EntryHash) -> ExternResult<Record> {
   ...
   set.insert(eh.clone());
   let query_args = ChainQueryFilter::default().include_entries(true).entry_hashes(set);
...
pub fn get_local_from_ah(ah: ActionHash) -> ExternResult<Record> {
   let query_args = ChainQueryFilter::default().include_entries(true);
