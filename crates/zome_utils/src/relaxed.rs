...
///
pub fn delete_link_relaxed(address: ActionHash, options: GetOptions) -> ExternResult<ActionHash> {
   HDK.with(|h| {
      h.borrow()
         .delete_link(DeleteLinkInput::new(address, options, ChainTopOrdering::Relaxed))
   })
}

///
pub fn delete_entry_relaxed(address: ActionHash) -> ExternResult<ActionHash> {
   HDK.with(|h| h.borrow().delete(DeleteInput::new(address, ChainTopOrdering::Relaxed)))
}
