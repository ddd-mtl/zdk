...
impl TypedAnchor {
   ...
   pub fn as_path(&self) -> TypedPath {
      TypedPath {
         link_type: ScopedZomeType {
            zome_index: ZomeIndex::from(self.zome_index),
...
pub fn batch_convert_path_to_anchor(tps: Vec<TypedPath>) -> ExternResult<Vec<TypedAnchor>> {
   ...
   for tp in tps {
      ...
      let Ok(str) = path2anchor(&tp.path) else {
         debug!("Failed to convert Path to Anchor: {:?}", tp.path);
         continue;
      };
      res.push(TypedAnchor::new(
         str,
         tp.link_type.zome_index.0,
         tp.link_type.zome_type.0,
      ));
   }
   Ok(res)
}
