...
pub fn send_shared_key(input: SendKeyInput) -> ExternResult<()> {
   ...
   let resp = call_remote(
      input.recipient,
      DEFAULT_COORDINATOR_ZOME_NAME,
      FunctionName("recv_shared_key".into()),
      None,
      send,
   )?;
...
pub fn recv_shared_key(input: RecvKeyInput) -> ExternResult<()> {
   ...
   let _ah = create_entry(SharedOwnershipEntry::SharedKey(SharedKey {
      key_ref: input.key.key_ref,
   }))?;
